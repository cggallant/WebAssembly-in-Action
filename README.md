...
The code in the following folder has been updated to work with _Emscripten 3.1.44_: [updated-code](https://github.com/cggallant/WebAssembly-in-Action/tree/master/updated-code)
...
Then, the commands in the EmccCommand files could be prefixed with the following: **docker run --rm -v "D:\WebAssembly-in-Action:/src" emscripten/emsdk:3.1.44**
  ...
  If you're using PowerShell, you can replace the **D:\WebAssembly-in-Action** portion of the path with _${pwd}_. For example, building the module for Chapter 3, section '3.4 html_template': 
    **docker run --rm -v ${pwd}:/src emscripten/emsdk:3.1.44 emcc calculate_primes.c -o html_template.html**
