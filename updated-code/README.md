...
Then, the commands in the EmccCommand files could be prefixed with the following: **docker run --rm -v "D:\WebAssembly-in-Action:/src" emscripten/emsdk:3.1.32**
  ...
  If you're using PowerShell, you can replace the **D:\WebAssembly-in-Action** portion of the path with $$ \lbrace \space pwd \space \rbrace. For example, building the module for Chapter 3, section '3.4 html_template': 
